...
def compare_checkpoints():
    baseline_pipeline = Flux1(
        model_config=ModelConfig.from_alias('schnell'),
        quantize_full_weights=None,
        local_path=None
    )


    baseline_pipeline.generate_image(seed=42,prompt="", config=Config(
    ...
    pipeline.generate_image(seed=42,prompt="",config=Config(
            num_inference_steps=1,
            height=1024,
            width=1920,
            guidance=3.5,
    ...
    for i, baseline in enumerate(baseline_outputs):
        ...
        similarity = (cosine_similarity_loss(
